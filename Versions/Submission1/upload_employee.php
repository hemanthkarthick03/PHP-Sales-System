...
$employee_download_excel_content = "
<?php
...
$employee_insert_success_content = "<!DOCTYPE html>
<html lang='en'>
<head>
    <meta charset='UTF-8'>
    <meta name='viewport' content='width=device-width, initial-scale=1.0'>
...
<body>
    <div class='container'>
        <h2 class='text-center mt-4'>Data Inserted Successfully!</h2>
        <div class='text-center mt-4'>
            <p>Your data has been successfully inserted into the database.</p>
            <a href='insert_form.php' class='btn btn-primary'>Insert More Data</a>
        </div>
    </div>
...
if (\$_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty(\$name) || empty(\$category) || \$val1 <= 0 || \$val2 <= 0 || \$val3 <= 0) {
        \$error = 'All fields are required and must be valid numbers greater than zero.';
    } else {
        // Calculate total
        \$total = \$val1 + \$val2 + \$val3;
...
<body>
    ...
    // Save the download sales data page
    $employee_download_excel_path = $employee_directory . '/' . 'download_excel.php';
    file_put_contents($employee_download_excel_path, $employee_download_excel_content);

    // Save the success page for Insertion of data
    $employee_insert_success_content_path = $employee_directory . '/' .'success.php';
    file_put_contents($employee_insert_success_content_path, $employee_insert_success_content);
    ...
    // Save the employee insert data page for insert form
    $employee_insert_data_content_path = $employee_directory . '/' . 'insert_data.php';
    file_put_contents($employee_insert_data_content_path, $employee_insert_data_content);
